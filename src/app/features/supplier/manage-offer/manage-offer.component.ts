  ...
  template: `
    <div class="container" style="max-width:900px;padding:2rem 1rem 4rem" role="main">
      ...
      <div class="card" style="padding:2rem">
        ...
        <div style="background:#F9FAFB;border-radius:12px;padding:1.5rem;display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:1rem">
          <div>
            <h4 style="font-weight:700;margin-bottom:0.375rem">Batch Status</h4>
            <p style="font-size:0.9375rem;color:var(--text-secondary);max-width:480px">This batch is currently open. Once the target is met or time expires, it will automatically close.</p>
          </div>
          <div style="display:flex;gap:0.75rem">
            <button class="btn btn-outline">Deactivate Offer</button>
            <button class="btn btn-primary" (click)="router.navigate(['/supplier/offers', offer().id])">Manage Details</button>
          </div>
        </div>
      </div>
    </div>
